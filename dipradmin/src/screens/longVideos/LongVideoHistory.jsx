...
import {
  ...
  deleteVersionOfLongVideoByVersionNumber,
...
function LongVideoHistoryPage() {
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const response = await getLongVideoHistoryById(videoId);
      if (response.success) {
  ...
  const handleDeleteVersion = async (versionNumber) => {
    try {
      const response = await deleteVersionOfLongVideoByVersionNumber(
        videoId,
        versionNumber
      );
      if (response.success) {
  ...
  const handleVersionClick = (version) => {
    navigate(`/edit-long-video/${videoId}?version=${version.versionNumber}`);
  };
  ...
  return (
    <div style={{ padding: "20px" }}>
      <Title level={3}>Long Video Version History</Title>
...
export default LongVideoHistoryPage;
