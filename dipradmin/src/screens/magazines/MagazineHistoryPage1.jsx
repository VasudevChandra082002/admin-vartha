...
function MagazineHistoryPage() {
  ...
  const fetchHistory = async () => {
    try {
      const response = await getMagazineHistory1ById(magazineId);
      if (response.success) {
  ...
  const handleDeleteVersion = async (versionNumber) => {
    ...
    try {
      const response = await deleteVersion1(magazineId, versionNumber);
      if (response.success) {
          ...
          renderItem={(item, index) => (
            <List.Item>
              ...
              >
                ...
                <Text strong>Updated At:</Text> {new Date(item.updatedAt).toLocaleString()}
              </Card>
            </List.Item>
          )}
        />
