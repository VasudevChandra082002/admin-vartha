...
function AddMagazinePage2() {
  ...
  const handleFormSubmit = async (values) => {
    if (!imageFile || !pdfFile) {
      message.error("Please upload both a thumbnail and a PDF before submitting.");
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        message.error(response.message || "Failed to add magazine.");
      }
    } catch (error) {
      console.error("Error submitting:", error);
      message.error("Error submitting magazine.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      >
        ...
        {/* RIGHT SIDE */}
        <Card title="General Information" style={{ flex: 2, minWidth: "400px" }}>
          <Form form={form} layout="vertical" onFinish={handleFormSubmit}>
            <Form.Item
              label="Title"
              name="title"
            ...
            <Form.Item
              name="publishedMonth"
              label="Published Month"
              rules={[{ required: true, message: "Select the published month!" }]}
            >
              <Select placeholder="Select month" allowClear>
                {monthOptions.map((month) => (
            ...
            >
              <Input placeholder="Enter year (e.g. 2024)" type="number" />
            </Form.Item>

            <Form.Item label="Magazine PDF" required>
              <Upload
