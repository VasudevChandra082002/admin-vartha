...
import { getVideoCategories, deleteVideoCategory } from "../../service/videoCategory/VideoCategoryApi";

function VideoCategoryTable() {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response?.success || response?.message?.includes("deleted")) {
        ...
        message.error(response?.message || "Failed to delete video category");
      }
    } catch (error) {
      message.error("Error deleting video category");
        ...
        return (
          <Space>
            ...
            <Button
              type="primary"
              icon={<EditOutlined />}
              onClick={() => navigate(`/video-category/edit/${recordId}`)}
            >
              Edit
            </Button>
            <Popconfirm
              title="Are you sure to delete this video category?"
...
export default VideoCategoryTable;
